...
app2 = Flask(__name__)
conn = MySQLdb.connect(host="localhost", user="root", password="root", db="mada")
...
def add_employee():
    ...
    if 'user_id' in session and in_user[7] == "a":
        ...
        if request.method == 'POST':
            ...
            if str(id_).isnumeric():
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM employee_work WHERE id = % s', (id_,))
                fetched_id = cursor.fetchall()
                print(fetched_id)
                cursor.close()
    ...
    if 'user_id' in session and in_user[7] == "a":
        ...
        if request.method == 'POST':
            ...
            if not bool(re.match('[0-9.\s]+$', salary)):
                ...
                cur.execute(
                    "UPDATE employee_work SET first_name  = %s,last_name  = %s, Branch_number = %s,start_date = %s,salary = %s,num_of_holidays = %s  WHERE Employee_id = %s",
                    (first_name, last_name, branch_num, s_date, salary, noh, emp_Id))
                conn.commit()
                cur.execute(
                    "UPDATE programmer SET skill  = %s, project_id = %s WHERE Employee_id = %s",
                    (skill, project_id,emp_Id))
                conn.commit()
                change_branch()
                flash('Employee Updated Successfully', 'success')
